...
\begin{frame}
  ...
  \vspace{5mm}
	...
	In defining propositional logic we suppressed a lot of detail into a single propositional variable $P$. It's time now to look at each proposition in more detail and parse out different components. \emph{Like physicists finding parts (particles) within their ``atoms''.}
...
\begin{frame}
	...
	We use the following standard language of \emph{first-order predicate logic} to build terms, well-formed formulae, and propositions.
...
\begin{frame}
	...
	Well-Formed Formulae (WFF) are defined inductively as follows: 
	...
	\textbf{These are the syntactic rules for building correct strings of symbols.}
...
\begin{frame}
	...
	\vspace{5mm}
	% I think the following paragraph is probably philosophically dubious... 
	% Meaning is a loaded word...
	% Defining a signature provides some meaning. But, some would say, if it doesn't specify a unique model then... 
	% What are we doing? Defining strings to interpret in ``models''? Or are we defining the very objects of mathematics? 
	They have no meaning, \emph{yet}. We don't work this abstractly. Instead we choose to construct specific base types with constants, functions, and predicates to express ideas and data precisely. It's this choice of ``signature'' that determines the meaning of the strings in this language. 
...
\begin{frame}
	...
	Bound and free variables act differently when interpreted; they play different roles. They distinguish between the propositions we want to prove and the merely syntactically correct formulae used to build those propositions.
...
\begin{frame}
	...
	% a[x/t] a[x/0+0] b[x/t] (! x not free) c[y/0]
...
\begin{frame}
	\frametitle{Terms, WFF, and Prop}
	
	We have so far distinguished two sorts of syntactically correct strings: terms and WFF. We are interested in \emph{proofs} and \emph{proving theorems.} With the notion of free/bounded variables we can now pick out those WFF which are propositions; Prop $\subset$ WFF. 
	
	\vspace{2mm}
	
	We say a WFF is closed if there are no free variables. Propositions are closed WFF. 
	
	\vspace{2mm}
	
	The distinction between free and bound variables is thus the distinction between propositions and the otherwise syntactically correct WFF. 
...
\begin{frame}
	...
	If $^{\Sigma}_{\forall x \alpha}\mathcal{D}$ is a derivation of $\forall x \ \alpha[x]$ from hypotheses $\Sigma$, where $y$ is free for $x$ in $\alpha$, then
...
\begin{frame}
	...
	% Note that careless way we are forced to deal with the domain when quantifiers are naked.
	% What is the domain? What is in it? At least Sherlock... but what else? 
...
\begin{frame}
	...
	Provide a proof to show $$\forall x (Px \rightarrow Qx), \hspace{0.5em} \forall x Px \hspace{0.5em} \vdash \hspace{0.5em} \forall x Qx$$
...
\begin{frame}
	...
	Provide a proof to show $$\forall x (Px \rightarrow Qx), \hspace{0.5em} \forall x (Qx \rightarrow Rx) \hspace{0.5em} \vdash \hspace{0.5em} \forall x (Px \rightarrow Rx)$$
...
\begin{frame}
	...
	Provide a proof to show $$\forall x \ Px \ \vdash \ \exists x \ Px$$
...
\begin{frame}
	...
	Provide a proof to show $$\forall x (Px \rightarrow Qx), \lnot Qa \ \vdash \ \exists x \ \lnot Px$$
...
\begin{frame}
	...
	% All we know is that there is *some* term out there that satisfies the predicate P. If we then argue that gamma follows from the existence of an *arbitrary* term satisfying the predicate P then we may conclude gamma. 
	% For, we know there is some term satisfying the predicate. 
...
\begin{frame}
	...
	% Take time to explain that if $y$ is arbitrary, then we can put in the term we know to exist from the first deduction. The fact that $y$ is arbitrary is encoded by not being free in $\gamma$ nor $\Sigma$. 
