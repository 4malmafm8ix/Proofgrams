...
% 	These caused debates based on logical and theological grounds\footnote{Amir Alexander, \emph{Infinitesimal: How a Dangerous Mathematical Theory Shaped the Modern World.}}. All of these debates came to a head in the nineteenth and twentieth centuries. 
...
\begin{frame}
  \frametitle{Direction}
	...
	Throughout the entire course we have been carrying around a set $\Sigma$ of hypotheses. Now we think of that set as including our axioms and the conclusions in our arguments as the theorems we hope to deduce from those axioms. 
\end{frame}

\begin{frame}
	\frametitle{First-Order Theories}
	
	Mathematics abounds with first-order theories. This formal language was adopted for much of mathematics in the early 1900s, following Hilbert's annoucement of his program. To the point that now you would be unlikely to find an advanced undergraduate course in mathemtaics that does not introduce the objects of interest (e.g. groups, rings, metric spaces) by specifying them as objects which obey some first-order axioms. 
...
\begin{frame}
	...
	is a deduction of $\alpha[s/t]$ from the hypotheses $\Sigma_{1} \cup \Sigma_{2}$. This states that identical terms may be substituted for identical terms. Unlike in forall introduction, in this case one can be selective about the instances of $s$ which are replaced by $t$. 
...
\begin{frame}
	...
	If we focus on the second argument, then there are only two cases to consider. Either $y = 0$, or there exists some other term $m$ such that $y = s \ m$. In the case that $y = 0$ the computation is clear: 
...
\begin{frame}
	...
	This is intended to represent $x + (m + 1)$
...
\begin{frame}
	...
	It might be helpful to recall this represents $x \times (m + 1)$
...
\begin{frame}
	...
	Modern developments in formal mathematics allow for more natural notations as these, without losing any of the rigour provided by the formal process. See Agda or Lean. 
\end{frame}
...
\begin{frame}
	...
	% With a nice straight forward example. 
\end{frame}
...
\begin{frame}
	...
	% Example: sqrt{n} constructible from compass, straight edge, unit length.
	%			Base case: sqrt(1) = 1 = unit 
	%			Induction step: given sqrt(n), 
	%							sqrt(n+1) is hypotenuse of sqrt n, 1
	%							right angle constructible by compass.
...
\begin{frame}
	...
	This presents the inductive mode of reasoning in the same manner as the modes we have formalised.
\end{frame}
...
\begin{frame}
	...
	Mathematicians and software engineers have developed higher order languages - so called Type Theories - that allow for the mathematician (and computer scientist) to write their proofs in a more natural syntax and then have them elaborated (analogous to compiler) down to these more formal natural deduction proof trees that we have been using. 
	
	% Agda: Natural Syntax -> Elaborator -> Formal Proof Tree -> Check!
