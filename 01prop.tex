...
\begin{frame}
	...
	Following 2000+ years of work developing all of the rich ideas that comprise modern mathematics, some began to wonder about the \emph{consistency} of the methods used. Indeed, there was debate about what ``\underline{the} methods of mathematics'' should be and camps were formed around these different philosophies. 
...
\begin{frame}
	...
	\textbf{Examples}
		...
		\begin{itemize}
			\item[] $x^{2} + y^{2} = z^{2}$ has infinitely many non-trivial solutions.
			\item[] $x^{5} + y^{5} = z^{5}$ has no non-trivial solutions.
			\item[] $\forall n \geq 3, x^{n} + y^{n} = z^{n}$ has no non-trivial solutions.
			\item[] $4$ is a prime number.
			\item[] $1 + 1 = 2$
			\item[] If a sequence of real numbers is bounded above, then it converges.
			\item[] If an increasing sequence of real numbers is bounded above, then it converges.
		\end{itemize}
...
\begin{frame}
	...
	Our language is further made up of \emph{well-formed formulae}, or \emph{propositions} which we define inductively as follows:
	...
	{\bf Notation:} We will refer to the totality of propositions as ``Prop'' and we will write ``$\alpha : \prop$'' to denote the fact that $\alpha$ is a proposition. 
...
\begin{frame}
	\frametitle{Examples}
	Which of the following are well formed in propositional logic?
	...
	\begin{itemize}
		...
		\item[7.] $\lnot Q$
		\item[8.] $A \lor Q$
		\item[9.] $A \rightarrow \lnot B \lor C$		
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	To disambiguate $\land$ and $\lor$ we group terms from the left. In this way, we say that $\land$ and $\lor$ associate to the left. On the other hand, implies $\to$ associates to the right.

	\begin{table}[h]
		\centering
		\begin{tabular}{c c c}
			$A \land B \lor C$ \hspace{10mm} & $A \to B \to C$ \hspace{10mm} & $A \land B \to \lnot C \lor D$ \\
			\hline
		\end{tabular}
	\end{table}
...
\begin{frame}
	...
	Using the binding convention above allows for each proposition to be parsed into a unique syntax tree. 

	\begin{table}[h]
		\centering
		\begin{tabular}{c c c}
			$A \land (B \lor C)$ \hspace{10mm} & $A \land \lnot B \land C$ \hspace{10mm} & $A \to B \lor C \land D$ \\
			\hline
		\end{tabular}
	\end{table}
	\vspace{50mm}
...
\begin{frame}
	...
	Later we will introduce more structure to our logic which will allow us to bring back the mathematical content. We are taking a birds-eye-view of the structure, later we will zoom into an atomic proposition and unpack the structure therein. 
...
\begin{frame}
	...
	{\bf Theorem:} $\gamma$ is a semantic consequence of $\Sigma$ if and only if $\gamma$ is a syntactic consequence of $\Sigma$. See Dirk van Dalen, \emph{Logic and Structure}. 	
\end{frame}
...
\begin{frame}
	...
	% Explain the proof technique in words; not yet natural deduction.
	\vspace{30mm}
...
\begin{frame}
	...
	% Now give the natural deduction.
...
\begin{frame}
	...
	We define (natural) deductions (or derivations, or proofs) inductively according to the following rules: 
...
\begin{frame}
	...
	So we will spend some time writing down the {\bf Rules of Inference} for each of the logical connectives. 
...
\begin{frame}
	...
	% Do NOT DO THIS: \alpha to \beta \vdash \beta
\end{frame}
...
\begin{frame}
	...
	One can may work backwards from conclusion to premises, as there is often only one move to make at any given step.
...
\begin{frame}
	...
	In general this process of weakening allows one to extend a proof of $\Sigma \ \vdash \beta$ to a proof of $\Sigma \ \vdash \alpha \to \beta$, for any proposition $\beta$. This extension is an application of implication introduction, where a (potentially) unused hypothesis $\alpha$ is discharged. 
	
	Although this does not necessarily capture what we intend by P implies Q, it does meet the definition as stated by the BHK:
  ...
  \end{center}
	...
	In this case the algorithm, or function, is simply the constant function that returns the known proof of $Q$ no matter which proof of $P$ we are given. 
	
	% Relevance/Relevant(?) logic attempts to address implication.

\end{frame}

\begin{frame}
	\frametitle{Deduction Meta-Theorem}	
...
\begin{frame}
	...
	% \begin{center}
	% 	$\begin{array}{c}
	% 		\infer[MP]{L}{\infer[MP\hspace{10mm}]{\lnot S \rightarrow L}{ P \rightarrow (\lnot S\rightarrow L) & P } 
	% 						&
...
\begin{frame}
	...
	$\lor$-elimination is more complicated that you may first think; students often make the following mistake: 
	...
	This is not $\lor$E. This rule of inference would be too powerful. With this rule, and the $\lor$ introduction rules above, we see that given any proposition $\alpha$, we can prove any other proposition $\beta$.
	
	\vspace{35mm}
	
	\textbf{Please, don't do this!}
...
\begin{frame}
	...
	If $^{\Sigma_{1}}_{\alpha\lor\beta}\mathcal{D}_{1}$, $^{\Sigma_{2}}_{\alpha\rightarrow\gamma}\mathcal{D}_{2}$, and $^{\Sigma_{3}}_{\beta\rightarrow\gamma}\mathcal{D}_{3}$ are derivations, then
...
\begin{frame}
  \frametitle{Example}
	...
	%One can given an altnerative proof where OR-E and AND-I steps are swapped.
\end{frame}

\begin{frame}
	\frametitle{Compact Notation and Subproofs}
...
\begin{frame}
	...
	Falsum $\bot$ is an atomic proposition which is to be thought of as denoting ``absurdity'' or ``contradiction''. It has no introduction rules. However, we will see it has an elimination rule.
...
\begin{frame}
	...
	Show $A \lor B, \lnot A, \lnot B \ \vdash C$
	\vspace{70mm}
...
\begin{frame}
	...
	\vspace{30mm}
	% Draw the rule for disjunction elimination. 
	% Draw the rule of implication elimination. 
	% Do the analogy with the rules for falsum. 
...
\begin{frame}
	\frametitle{Example}
	Show $A \rightarrow B, \hspace{0.20mm} A \rightarrow \lnot B \ \vdash \lnot A$
	\vspace{7cm}
...
\begin{frame}
	\frametitle{Shape of RAA Proofs} 
	
	% Note here the unusual shape of RAA. 
	% We are to prove A 
	% We assume notA and Psi; which yields a contradiction. 
	% Infer not psi; this also yields contradiction. 
	% Now we psi has been dischaged and RAA yields A. 
	% This suggests we just prove from Psi or notPsi i.e. LEM.
\end{frame}
...
\begin{frame}
	\frametitle{LEM as Rule of Inference}
	
	Instead of RAA, we can define Classical logic as the extension of Intuitionistic logic by the addition of the following rule of inference. Notice there is no deduction above $\alpha \lor \lnot\alpha$ - this rule of inference is saying you can assume that for any and all $\alpha : $ Prop. 
	...
	\begin{center}		
		$\begin{array}{c}		
		\infer[\LEM]{\beta}
		{\raisebox{-3ex}{$\alpha \lor \lnot \alpha$}
		&
		\begin{array}{c} \Sigma_{1} \\ \mathcal{D} \\ \alpha \to \beta \end{array}
		&
		\begin{array}{c} \Sigma_{2} \\ \mathcal{D} \\ \lnot\alpha \to \beta \end{array}}
		\end{array}$
	\end{center}
...
\begin{frame}
	\frametitle{Pierce's Law}
	
	Show $\vdash \ ((A \to B) \to A) \to A$
...
\begin{frame}
	\frametitle{RAA = Pierce}
	
	We can phrase Pierce's Law as a new rule of inference: 
	
	\begin{center}		
		$\begin{array}{c}		
		\infer[\text{Pierce},n]{\alpha}
		{\begin{array}{c} \Sigma, \ \alpha \to \beta^{1} \\ \mathcal{D} \\ \alpha \end{array}}
		\end{array}$
	\end{center}
	
	Whenever we see this pattern - note that $\beta$ can be any Prop - we can discharge the hypothesis $\alpha \to \beta$. Although the conclusion does not change, the hypotheses do.
	
	\vspace{5mm}
	
	From intuitionistic logic we can add either RAA or Pierce to our logic. In either case, we get exactly the same set of theorems. We can show this by deriving RAA from Pierce, and deriving Pierce from RAA. 

\end{frame}

\begin{frame}
	\frametitle{RAA implies Pierce}

	Suppose we have a deduction $\mathcal{D}^{\alpha \to \beta}_{\alpha}$. We can use RAA to extend this to a proof of $\alpha$; thus showing Pierce can be derived from RAA.
...
\begin{frame}
	\frametitle{Pierce implies RAA}

	Suppose we have a deduction $\mathcal{D}^{\lnot \alpha}_{\bot}$. We can use Pierce to extend this to a proof of $\alpha$; thus showing RAA can be derived from Pierce.
...
\begin{frame}
  \frametitle{Departure from BHK}
	...
	The inclusion of RAA allows for proofs of some apparently harmless theorems like DNE and LEM. However this power is not without its consequences...
...
\begin{frame}
	...
	Choice of logic (i.e. rules of inference) is left to the mathematician. Therefore, we should be more specific when we assert one Prop is a syntactic consequence of a set $\Sigma$. This is a \emph{relative} notion and so one should quote the (weakest) logic used in the derivation. 
...
\begin{frame}
  \frametitle{Logical Equivalence}
	...
	Logical equivalence is an equivalence relation on Prop.

\end{frame}

\begin{frame}
	\frametitle{Theorems}
	We say a well-formed formula $\alpha$ is a theorem if there exists a natural deduction $\mathcal{D}$ from no assumptions i.e. $\Sigma = \emptyset$ and we denote this as $\vdash \alpha$.
...
\begin{frame}
	\frametitle{True, False, and Truth Tables}
	
	In a course on discrete mathematics (e.g. MATH120) you will be introduced to logic from the point-of-view of propositions being True or False. You will determine the truth values by filling out a truth table. You would judge an argument valid:	
	$$\Sigma \ \vDash \ \gamma$$	
	If whenever all hypotheses of $\Sigma$ are True, the conclusion $\gamma$ is also True. Look at all the rows in the truth table where each hypothesis in $\Sigma$ is True and check $\gamma$ is also True in those cases. 
	
	\textbf{Question:} How does this Truth point-of-view compare to the Proof point-of-view that we have studied?	
	$$\text{Soundness of ND: If } \Sigma \ \vdash \ \gamma \text{, then } \Sigma \ \vDash \ \gamma$$	
	$$\text{Completeness of Classical ND: If } \Sigma \ \vDash \ \gamma \text{, then } \Sigma \ \vdash_{K} \ \gamma$$
