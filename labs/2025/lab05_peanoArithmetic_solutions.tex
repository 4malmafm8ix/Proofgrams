...
$$ \PA \ \vdash \ \exists y \ (n=s(y)) \to [s(n) = 0 \lor \exists y (s(n) = s(y))]$$
	...
	\item Provide natural deductions of the following theorems of Peano Arithmetic. Beware each of these must be true, but some I have not provided natural deductions for. Some may require breaking down into further subgoals (lemma) to help. I recommend writing informal proofs, before formalising them with natural deductions.
		...
		\begin{enumerate}
			...
			\begin{center}
			$\begin{array}{c}
				\infer[\text{SUGAR}]{a < s(a)}
					{\infer[\exists I]{\exists x \ a + s(x) = s(a)}
						{\infer[\forall E]{a + s(0) = s(a)}
							{\infer[\THM]{\forall x \ x + 1 = s(x)}
								{}}}}
			\end{array}$			
			\end{center}
			...
			\footnotesize{$\begin{array}{c}
				\infer[\exists E]{a < b}
					{s(a) < s(b)
					&
					\infer[\to I,1]{s(b) = s(a) + s(t) \to a < b}
						{\infer[\text{SUGAR}]{a < b}
							{\infer[\MP]{b = a + s(t)}
								{\infer[\forall E]{s(b) = s(a + s(t)) \to b = a + s(t)}
									{\PA 2}
								&
								\infer[=E]{s(b) = s(a + s(t))}
									{\infer[1]{s(b) = s(a) + s(t)}{}
									&
									\infer[\forall E]{s(a) + s(t) = s(a + s(t))}{\infer[\THM]{\forall x \ \forall y \ s(x) + y = s(x + y)}
										{}}}}}}}
			\end{array}$}
			\end{center}
