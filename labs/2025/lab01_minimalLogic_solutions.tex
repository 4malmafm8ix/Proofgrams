...
\noindent\fbox{
	\parbox{\textwidth}{
		...
		Relevant Reading: \href{https://leanprover-community.github.io/logic_and_proof/index.html}{L$\exists\forall$N Chapters 3,4} and \href{https://www.cs.cornell.edu/courses/cs6110/2015sp/textbook/Simon\%20Thompson\%20textbook.pdf}{Simon section 1.1} 
...
\begin{center}
	...
	\begin{tabular}{c c}
		...
		&
		$\begin{array}{c}\infer[\lor I_{R}]{B \lor A}
		{\begin{array}{c} \Sigma \\ \vdots \\ A \end{array}}\end{array}$
	\end{tabular}\\
...
\begin{center}
	\begin{tabular}{c c c}
		\begin{tabular}{c c}
			$\begin{array}{c}\infer[\land E_{L}]{A}
			{\begin{array}{c} \Sigma \\ \vdots \\ A \land B \end{array}}\end{array}$
			&
			$\begin{array}{c}\infer[\land E_{R}]{B}
			{\begin{array}{c} \Sigma \\ \vdots \\ A \land B \end{array}}\end{array}$
		\end{tabular}

	&

	$\begin{array}{c}\infer[\to E]{B}
		{\begin{array}{c} \Sigma \\ \vdots \\ A \end{array}
		&
			...
			$\begin{array}{c}
				\infer[\lor E]{(A \land B) \lor (A \land C)}
					...
					\infer[\to I, 1]{B \to ((A \land B) \lor (A \land C))}
						{\infer[\lor I_{L}]{(A \land B) \lor (A \land C)}{\infer[\land I]{A \land B}
								{\infer[\land E_{L}]{A}
									{A \land (B \lor C)}
							&
						\temp{1}{B}}}}
					&
					\infer[\to I,2]{C \to ((A \land B) \lor (A \land C))}{\infer[\lor I_{R}]{(A \land B) \lor (A \land C)}{\infer[\land I]{A \land C}
						{\infer[\land E_{L}]{A}{A \land (B \lor C)}
							&
						\temp{2}{C}}}}
						}
		...
		\begin{center}
			$\begin{array}{c}
				\infer[\land I]{(A \lor B) \land (A \lor C)}
					{\infer[\lor I_{L}]{A \lor B}{\temp{1}{A}}
					&
					\infer[\lor I_{L}]{A \lor C}{\temp{1}{A}}}
			\end{array}$
		\end{center}
